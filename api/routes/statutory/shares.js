...
router.get('/:companyId?', auth, async (req, res) => {
  try {
    ...
    const shares = await getPrismaClient().share.findMany({
      where,
      orderBy: { class: 'asc' },
      include: {
        company: {
...
router.get('/:companyId/:id', auth, async (req, res) => {
  try {
    const share = await getPrismaClient().share.findFirst({
      where: { 
        AND: [
          { id: req.params.id },
...
router.post('/:companyId', auth, async (req, res) => {
  try {
    // Extract only the fields that are in the Share model
    const {
      class: className,
      type,
      nominalValue,
    ...
    const share = await getPrismaClient().share.create({
      data: {
        class: className,
        type,
        nominalValue: parseFloat(nominalValue),
        currency,
        votingRights: Boolean(votingRights),
        dividendRights: Boolean(dividendRights),
...
router.put('/:companyId/:id', auth, async (req, res) => {
  try {
    ...
    // First find the share and verify ownership
    const existingShare = await getPrismaClient().share.findFirst({
      where: {
        AND: [
          { id: req.params.id },
          { companyId: req.params.companyId }
    ...
    const share = await getPrismaClient().share.update({
...
router.delete('/:companyId/:id', auth, async (req, res) => {
  try {
    ...
    const share = await getPrismaClient().share.delete({
      where: { id: req.params.id }
    });
    ...
    await getPrismaClient().activity.create({
      data: {
        ...
        entityType: 'share',
        entityId: share.id,
        description: `Share class '${share.class}' deleted`,
...
router.get('/:companyId/export/excel', auth, async (req, res) => {
  try {
    ...
    const data = shares.map(share => ({
      'Class': share.class,
      'Type': share.type,
      'Nominal Value': share.nominalValue,
      'Currency': share.currency,
      'Voting Rights': share.votingRights ? 'Yes' : 'No',
      'Dividend Rights': share.dividendRights ? 'Yes' : 'No',
      'Transferable': share.transferable ? 'Yes' : 'No',
      'Total Issued': share.totalIssued,
      'Status': share.status
    }));
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=shares.xlsx');
...
router.get('/:companyId/export/pdf', auth, async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=shares.pdf');
    ...
    if (shares[0]?.company) {
      doc.fontSize(16).text(shares[0].company.name || shares[0].company.legalName, { align: 'center' });
      doc.moveDown();
    }

    // Add title
    doc.fontSize(14).text('Share Classes Register', { align: 'center' });
    doc.moveDown();
    ...
    shares.forEach((share, index) => {
      doc.fontSize(12).text(`${index + 1}. ${share.class}`);
      doc.fontSize(10)
        .text(`Type: ${share.type}`)
        .text(`Nominal Value: ${share.nominalValue} ${share.currency}`)
        .text(`Voting Rights: ${share.votingRights ? 'Yes' : 'No'}`)
        .text(`Dividend Rights: ${share.dividendRights ? 'Yes' : 'No'}`)
        .text(`Transferable: ${share.transferable ? 'Yes' : 'No'}`)
        .text(`Total Issued: ${share.totalIssued}`)
...
router.post('/:companyId/confirm-import', auth, upload.single('file'), async (req, res) => {
  try {
    ...
    const job = await importQueue.add({
      ...
      entityType: 'share' // Explicitly set entityType
    }, {
      attempts: 3,
      backoff: {
        type: 'exponential',
...
router.get('/:companyId/import-status/:jobId', auth, async (req, res) => {
  try {
    ...
    logger.debug('Import status check:', {
      ...
      currentShare: progressData?.currentShare
    });
