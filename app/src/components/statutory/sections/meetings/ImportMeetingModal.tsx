...
const ImportMeetingModal: React.FC<ImportMeetingModalProps> = ({ show, onHide, onSuccess, companyId }) => {
  ...
  const requiredColumns = [
    ...
    { key: 'quorumRequired', label: 'Quorum Required' },
    ...
    const sampleData = [
      {
        ...
        agenda: 'Annual Report Review, Financial Statements, Director Appointments'
      }
    ];
  ...
  const onFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      console.log('Sending request to:', `/statutory/meetings/${companyId}/read-headers`);
      ...
      const response = await api.post(`/statutory/meetings/${companyId}/read-headers`, formData, {
        headers: {
  ...
  const handleImportSuccess = useCallback((count: number) => {
    ...
    toast.success(`Successfully imported ${count} meetings`);
  ...
  const confirmImport = async () => {
    ...
    try {
      const response = await api.post(`/statutory/meetings/${companyId}/confirm-import`, formData);
  ...
  const checkImportStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(`/statutory/meetings/${companyId}/import-status/${currentJobId}`);
      ...
      if (response.data.state === 'failed') {
        ...
        const meeting = response.data.currentMeeting;
        ...
        setLoadingMessage(`Importing meetings... ${progress}%`);
      }
    } catch (err: any) {
      ...
      if (err.response?.status === 404) {
        ...
        try {
          const finalCheck = await api.get(`/statutory/meetings/${companyId}/import-status/${currentJobId}`);
        ...
        setLoadingMessage(`Importing meetings... ${simulatedProgress}%`);
      }
    }
  ...
  return (
    <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false} size="lg">
      <Modal.Header closeButton={!loading || importSuccess}>
        <Modal.Title>Import General Meetings</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {renderStepIndicator()}
        ...
        {currentStep === 1 && (
          <>
            <p className="mb-4">
              To import general meetings, please follow these steps:
            </p>
            <ol className="mb-4">
              <li className="mb-2">
              ...
              <li className="mb-2">Fill in your meeting data in the CSV template</li>
                ...
                <tbody>
                  {previewData.slice(0, 5).map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>{item.quorumRequired}</td>
                      <td>{item.quorumPresent}</td>
          ...
          <>
            <div className="text-center py-4">
              ...
              <p className="mb-4" style={{ fontSize: '1.1rem' }}>
                You are about to import {previewData.length} meetings. This action cannot be undone.
              </p>
            </div>
          </>
        )}
          ...
          <>
            <div className="text-center py-4">
              <div className="mb-4">
                ...
                <p className="text-muted mb-4">
                  Successfully imported {importedCount} meetings.
                </p>
              </div>
              <div className="alert alert-info">
                <small>
                  The meetings list has been updated. You can now close this window.
                </small>
              </div>
            </div>
          </>
...
export default ImportMeetingModal;
