...
const ImportShareholderModal: React.FC<ImportShareholderModalProps> = ({ show, onHide, onSuccess, companyId }) => {
  ...
  const requiredColumns = [
    ...
    { key: 'ordinaryShares', label: 'Ordinary Shares' },
    { key: 'preferentialShares', label: 'Preferential Shares' },
  ...
  const downloadSampleFile = (e: React.MouseEvent) => {
    ...
    const sampleData = [
      {
        ...
        ordinaryShares: '1000',
        preferentialShares: '500',
        dateAcquired: '01/01/2025',
        status: 'Active'
      }
    ];
    ...
    const csvContent = [
      ...
      ...sampleData.map(item => [
        ...
        escapeCSV(item.ordinaryShares),
        escapeCSV(item.preferentialShares),
        escapeCSV(item.dateAcquired),
    ...
    a.href = url;
    a.download = 'shareholders_template.csv';
  ...
  const onFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await api.post(`/statutory/shareholders/${companyId}/read-headers`, formData);
  ...
  const handleImportSuccess = useCallback((count: number) => {
    ...
    toast.success(`Successfully imported ${count} shareholders`);
    onSuccess();
    onHide();
  }, [onSuccess, onHide]);
  ...
  const confirmImport = async () => {
    ...
    try {
      const response = await api.post(`/statutory/shareholders/${companyId}/confirm-import`, formData);
  ...
  const checkImportStatus = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/statutory/shareholders/${companyId}/import-status/${currentJobId}`);
      ...
      if (response.data.state === 'failed') {
        ...
        setCurrentShareholder(response.data.currentShareholder || '');
        setLoadingMessage(`Importing shareholders... ${progress}%`);
      }
    } catch (err: any) {
      if (err.response?.status === 404) {
        ...
        setLoadingMessage(`Importing shareholders... ${simulatedProgress}%`);
      }
    }
    ...
    <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false} size="xl">
      ...
      <Modal.Body>
        ...
        {currentStep === 1 && (
          <>
            <p className="mb-4">
              To import shareholders, please follow these steps:
            </p>
            <ol className="mb-4">
              <li className="mb-2">
              ...
              <li className="mb-2">Fill in your shareholder data in the CSV template</li>
          ...
          <>
            <div className="text-center py-4">
              <div className="mb-4">
                ...
                <p className="text-muted mb-4">
                  Successfully imported {importedCount} shareholders.
                </p>
              </div>
              <div className="alert alert-info">
                <small>
                  The shareholders list has been updated. You can now close this window.
                </small>
              </div>
            </div>
          </>
...
export default ImportShareholderModal;
