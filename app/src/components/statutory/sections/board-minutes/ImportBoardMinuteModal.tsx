...
const ImportBoardMinuteModal: React.FC<ImportBoardMinuteModalProps> = ({ show, onHide, onSuccess, companyId }) => {
  ...
  const downloadSampleFile = (e: React.MouseEvent) => {
    ...
    const sampleData = [
      {
        ...
        startTime: '10:00',
        endTime: '11:30',
        venue: '15 St. Stephen\'s Green, Dublin 2',
        chairperson: 'John Smith',
        attendees: 'John Smith, Jane Doe, Michael Johnson',
        agenda: 'Q4 Results Review, Strategic Planning',
        minutes: 'The board reviewed Q4 results and discussed strategic plans for 2025.',
        status: 'Draft',
        attachments: 'Q4-Results.pdf, Strategic-Plan.pdf',
    ...
    const csvContent = [
      ...
      ...sampleData.map(item => [
        escapeCSV(item.minuteId),
        escapeCSV(item.meetingDate),
        escapeCSV(item.startTime),
        escapeCSV(item.endTime),
        escapeCSV(item.venue),
        escapeCSV(item.chairperson),
        escapeCSV(item.attendees),
        escapeCSV(item.agenda),
        escapeCSV(item.minutes),
        escapeCSV(item.status),
        escapeCSV(item.attachments),
  ...
  const onFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      console.log('Sending request to:', `/statutory/board-minutes/${companyId}/read-headers`);
      ...
      const response = await api.post(`/statutory/board-minutes/${companyId}/read-headers`, formData, {
        headers: {
  ...
  const previewImport = async () => {
    ...
    try {
      ...
      const response = await api.post(`/statutory/board-minutes/${companyId}/preview-import`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        },
        maxContentLength: Infinity,
  ...
  const handleImportSuccess = useCallback((count: number) => {
    ...
    toast.success(`Successfully imported ${count} board minutes`);
  ...
  const confirmImport = async () => {
    ...
    try {
      const response = await api.post(`/statutory/board-minutes/${companyId}/confirm-import`, formData);
  ...
  const checkImportStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(`/statutory/board-minutes/${companyId}/import-status/${currentJobId}`);
      ...
      if (response.data.state === 'failed') {
        ...
        const minute = response.data.currentMinute;
        ...
        setLoadingMessage(`Importing board minutes... ${progress}%`);
      }
    } catch (err: any) {
      ...
      if (err.response?.status === 404) {
        ...
        try {
          const finalCheck = await api.get(`/statutory/board-minutes/${companyId}/import-status/${currentJobId}`);
        ...
        setLoadingMessage(`Importing board minutes... ${simulatedProgress}%`);
      }
    }
  ...
  return (
    <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false} size="lg">
      <Modal.Header closeButton={!loading || importSuccess}>
        <Modal.Title>Import Board Minutes</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {renderStepIndicator()}
        ...
        {currentStep === 1 && (
          <>
            ...
            <ol className="mb-4">
              ...
              <li className="mb-2">Fill in your board minute data in the CSV template</li>
          ...
          <>
            <div className="text-center py-4">
              ...
              <p className="mb-4" style={{ fontSize: '1.1rem' }}>
                You are about to import {previewData.length} board minutes. This action cannot be undone.
              </p>
            </div>
          </>
        )}
          ...
          <>
            <div className="text-center py-4">
              <div className="mb-4">
                ...
                <p className="text-muted mb-4">
                  Successfully imported {importedCount} board minutes.
                </p>
              </div>
              <div className="alert alert-info">
                <small>
                  The board minutes list has been updated. You can now close this window.
                </small>
              </div>
            </div>
          </>
...
export default ImportBoardMinuteModal;
