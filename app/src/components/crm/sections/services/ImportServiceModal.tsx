...
const ImportServiceModal: React.FC<ImportServiceModalProps> = ({
  show,
  onHide,
  onSuccess,
  companyId
  ...
  const downloadTemplate = async () => {
    try {
      const response = await api.get('/crm/services/import/template', {
        responseType: 'blob'
      });
      ...
      link.href = url;
      link.setAttribute('download', 'services_import_template.csv');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/crm/services/${companyId}/import`, formData, {
        headers: {
      ...
      setError(err.response?.data?.message || 'Failed to import services. Please check your file and try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Modal.Body className="bg-light p-4">
          ...
          <div className="text-center mb-4 py-4">
            ...
            <h5>Upload Services CSV File</h5>
            <p className="text-muted">
              Import your services using our CSV template format
            </p>
          </div>
          ...
          <div className="alert alert-info shadow-sm">
            ...
            <ul className="mb-0">
              ...
              <li>Ensure all required fields are filled (name, description, category)</li>
              <li>For pricing tiers, use the format: BASIC:price,STANDARD:price,PREMIUM:price</li>
              <li>Features should be separated by semicolons</li>
              <li>Status should be one of: Active, Inactive, Archived</li>
            </ul>
          </div>
        </Modal.Body>
        <Modal.Footer>
