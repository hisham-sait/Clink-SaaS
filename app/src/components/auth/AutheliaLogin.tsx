...
const AutheliaLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First check if Authelia is available
      try {
        const response = await fetch('http://localhost:9091/api/state', { 
          method: 'GET',
          mode: 'no-cors' // This allows us to at least attempt the request
        });
        
        // If we get here, Authelia might be available
        // Redirect to Authelia login endpoint
        const autheliaUrl = new URL('http://localhost:9091/api/verify');
        
        // Add return URL as a parameter
        const returnUrl = new URL(window.location.origin);
        returnUrl.pathname = '/auth/callback';
        autheliaUrl.searchParams.append('rd', returnUrl.toString());
        
        // Store credentials in session storage for the callback
        sessionStorage.setItem('authelia_credentials', JSON.stringify({
        ...
        // Authelia is not available, fall back to regular login
        ...
        <div className="card-body">
          <div className="text-center mb-4">
            <img 
              ...
              style={{ height: '40px' }} 
            />
            <h2 className="fw-bold">Login with Authelia</h2>
          </div>
