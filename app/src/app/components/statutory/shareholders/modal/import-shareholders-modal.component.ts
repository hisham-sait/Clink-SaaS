...
@Component({
  selector: 'app-import-shareholders-modal',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="modal-header">
      <h4 class="modal-title">Import Shareholders</h4>
    ...
    <div class="modal-body">
      ...
      <div *ngIf="currentStep === 1">
        ...
        <ol class="mb-4">
          ...
          <li>Fill in your shareholder data following the template format</li>
          ...
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              <tr *ngFor="let item of previewData.slice(0, 5)">
                ...
                <td>{{ item.lastName }}</td>
                <td>{{ item.shares?.ordinary || 0 }}</td>
                <td>{{ item.shares?.preferential || 0 }}</td>
...
export class ImportShareholdersModalComponent implements OnInit {
  currentStep = 1;
  loading = false;
  loadingMessage = '';
  error: string | null = null;
  previewData: Partial<Shareholder>[] = [];
  companyId!: string;
  ...
  downloadSampleFile(event: Event): void {
    ...
    const csvContent = [
      ...
      ...sampleData.map(item => [
        ...
        item.email,
        item.phone,
        item.shares.ordinary,
        item.shares.preferential,
        item.dateAcquired,
        item.status
      ].join(','))
    ].join('\n');
  ...
  onFileSelected(event: Event): void {
    ...
    this.shareholderService.previewImport(this.companyId, formData)
      .pipe(
        catchError(error => {
      ...
      .subscribe((result: { data: Partial<Shareholder>[] }) => {
  ...
  confirmImport(): void {
    this.loading = true;
    this.loadingMessage = 'Importing shareholders...';
    this.error = null;

    // Proceed with the actual import
    this.shareholderService.confirmImport(this.companyId)
      .pipe(
        catchError(error => {
          this.error = error.error?.message || 'Failed to import shareholders. Please try again.';
          return of(null);
        }),
        finalize(() => {
          this.loading = false;
