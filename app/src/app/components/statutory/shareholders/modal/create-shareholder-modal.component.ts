...
@Component({
  ...
  template: `
    <div class="modal-header">
      <h5 class="modal-title">Add New Shareholder</h5>
...
export class CreateShareholderModalComponent {
  ...
  constructor(
    ...
    this.shareholderForm = this.fb.group({
      ...
      dateAcquired: [new Date().toISOString().split('T')[0], Validators.required],
      status: ['Active']
    });
  }
  ...
  onSubmit(): void {
    if (this.shareholderForm.valid) {
      ...
      this.activeModal.close(shareholder);
    } else {
