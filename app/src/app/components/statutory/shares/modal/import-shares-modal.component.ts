...
@Component({
  selector: 'app-import-shares-modal',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="modal-header">
      <h4 class="modal-title">Import Shares</h4>
    ...
    <div class="modal-body">
      ...
      <div *ngIf="currentStep === 1">
        <p class="mb-4">
          To import shares, please follow these steps:
        </p>
        <ol class="mb-4">
          <li>
          ...
          <li>Fill in your share data following the template format</li>
          ...
          <table class="table table-sm table-bordered">
            ...
            <tbody>
              <tr *ngFor="let item of previewData.slice(0, 5)">
                ...
                <td>{{ item.currency }}</td>
                <td>{{ item.votingRights ? 'Yes' : 'No' }}</td>
                <td>{{ item.status }}</td>
              </tr>
            </tbody>
          </table>
...
export class ImportSharesModalComponent implements OnInit {
  currentStep = 1;
  loading = false;
  loadingMessage = '';
  error: string | null = null;
  previewData: Partial<Share>[] = [];
  companyId!: string;
    ...
    const sampleData = [
      {
        ...
        totalIssued: 1000,
        status: 'Active',
        description: 'Class A Ordinary Shares with full voting and dividend rights'
      },
      {
        class: 'B',
        type: 'Preferential',
        ...
        totalIssued: 500,
        status: 'Active',
        description: 'Class B Preference Shares with fixed dividend rights'
      }
    ];
  ...
  onFileSelected(event: Event): void {
    ...
    this.shareService.previewImport(this.companyId, formData)
      .pipe(
        catchError(error => {
      ...
      .subscribe((result: { data: Partial<Share>[] }) => {
  ...
  confirmImport(): void {
    this.loading = true;
    this.loadingMessage = 'Importing shares...';
    this.error = null;

    // Proceed with the actual import
    this.shareService.confirmImport(this.companyId)
      .pipe(
        catchError(error => {
          this.error = error.error?.message || 'Failed to import shares. Please try again.';
          return of(null);
        }),
        finalize(() => {
          this.loading = false;
