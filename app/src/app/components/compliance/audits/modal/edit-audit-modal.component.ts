    ...
    <form [formGroup]="auditForm" (ngSubmit)="onSubmit()">
      <div class="modal-body">
        ...
        <div class="mb-3">
          ...
          <div class="form-text text-danger" *ngIf="auditForm.get('status')?.touched && auditForm.get('status')?.errors?.['required']">
            Status is required
          </div>
        </div>
...
export class EditAuditModalComponent implements OnInit {
  @Input() audit!: Audit;
  ...
  ngOnInit(): void {
    if (this.audit) {
      ...
      const startDate = new Date(this.audit.startDate).toISOString().split('T')[0];
      const endDate = new Date(this.audit.endDate).toISOString().split('T')[0];
