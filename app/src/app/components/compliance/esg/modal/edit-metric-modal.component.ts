...
@Component({
  selector: 'app-edit-metric-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbModule],
  template: `
    <div class="modal-header">
      <h5 class="modal-title">Edit ESG Metric</h5>
    ...
    <form [formGroup]="metricForm" (ngSubmit)="onSubmit()">
      <div class="modal-body">
        ...
        <div class="mb-3">
          ...
          <div class="form-text text-danger" *ngIf="metricForm.get('status')?.touched && metricForm.get('status')?.errors?.['required']">
            Status is required
          </div>
        </div>
...
export class EditMetricModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.metric) {
      ...
      const nextReportDue = new Date(this.metric.nextReportDue).toISOString().split('T')[0];
