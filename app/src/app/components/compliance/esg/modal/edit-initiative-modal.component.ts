...
@Component({
  selector: 'app-edit-initiative-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbModule],
  template: `
    <div class="modal-header">
      <h5 class="modal-title">Edit ESG Initiative</h5>
      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="col-md-4 mb-3">
            ...
            <div class="input-group">
              <span class="input-group-text">$</span>
              <input type="number" class="form-control" formControlName="spent">
            </div>
          </div>
        </div>
...
export class EditInitiativeModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.initiative) {
      ...
      const startDate = new Date(this.initiative.startDate).toISOString().split('T')[0];
      const endDate = new Date(this.initiative.endDate).toISOString().split('T')[0];

      // Convert arrays to string for textareas
      const stakeholders = this.initiative.stakeholders.join('\n');
      const metrics = this.initiative.metrics.join('\n');
