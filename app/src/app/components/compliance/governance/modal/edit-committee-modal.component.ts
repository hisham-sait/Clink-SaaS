...
@Component({
  selector: 'app-edit-committee-modal',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbModule],
  template: `
    <div class="modal-header py-2">
      <h5 class="modal-title fs-6">Edit Committee</h5>
    ...
    <form [formGroup]="committeeForm" (ngSubmit)="onSubmit()">
      <div class="modal-body p-3">
        <div class="row g-3">
          ...
          <!-- Review Date -->
          <div class="col-12">
            <label class="form-label small">Next Review Date</label>
            <input type="date" class="form-control form-control-sm" formControlName="nextReviewDate">
          </div>
        </div>
      </div>
...
export class EditCommitteeModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.committee) {
      this.selectedMembers = [...this.committee.members];
      this.committeeForm.patchValue({
        ...this.committee,
        responsibilities: this.committee.responsibilities.join('\n'),
